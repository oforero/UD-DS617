...
#
# This goal is to find the top 10 most used tags 
#
# I decided to use a simple mapper that for each post returns: tag '\t' 1  
...
def print_output(key, vals):
    for v in vals:
        print "{0}\t{1}".format(v.upper(), 1)
####
...
for line in reader:
    if len(line) == 19:
       ...
       if is_not_header(key): # Skip the header line in case it exists
           val = post_process_value(line[tagnames])
